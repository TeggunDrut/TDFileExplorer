class NAnd extends NodeElement {
  constructor(x, y, w, h, name) {
    super(x, y, w, h, name);
    this.nodes = [
      new Node(this, 0, this.h / 2 - 100 / 4, 7, false),
      new Node(this, 0, this.h / 2 + 100 / 4, 7, false),
    ...
    nodes.push(this.nodes[0], this.nodes[1], this.nodes[2]);
  }

  updateNodes(mouse) {
    // case: 0, 0
    if (this.nodes[0].powered == false && this.nodes[1].powered == false) {
    ...
    // case 1, 0 or 0, 1
    if (this.nodes[0].powered && this.nodes[1].powered == false
      || this.nodes[0].powered == false && this.nodes[1].powered) {
      this.nodes[2].powered = true;
    }
    let count = 0;
    if (mouse) {
